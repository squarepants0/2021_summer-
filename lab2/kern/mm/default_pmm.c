 ...
 *              The struct free_area_t is(双向链表表头和计数器) used for the management of free mem blocks. At first you should
 ...
 *                  初始化每个表项：flags置PG_property：1，可用
 *                  p->flags should be set bit PG_property (means this page is valid. In pmm_init fun (in pmm.c),
 *                  the bit PG_reserved is setted in p->flags)
 *                  if this page  is free and is not the first page of free block, p->property should be set to 0. 是否描述总空闲块的第一个
